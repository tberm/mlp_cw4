...
def get_batch_of_embeddings(number=None, split='train', layer=-1):
    rng = np.random.default_rng(17)
    frames = []
    folder = Path('../llama-true-false-results') / split #remove the ../ depending on where it is called from
    if isinstance(layer, int):
        pattern = f'*{str(layer)}.csv'
    else:
        pattern = '*.csv'
    ...
    for csv_path in folder.glob(pattern):
        ...
        source = str(csv_path).split('/')[-1].split('.')[0]
        frame['source'] = source
        frames.append(frame[['statement', 'label', 'embeddings', 'source']])
